...
const getAllPlans = async (filters = {}) => {
  const { branchId, type, active, adminId } = filters;
...
const createPlan = async (planData, userId) => {
  const { name, type, sessions, validity, price, branchId } = planData;
...
const updatePlan = async (id, planData, userId, branchId = null) => {
  const { name, type, sessions, validity, price, active } = planData;
