...
const getTrainers = async (req, res, next) => {
  try {
    ...
    const trainers = await classScheduleService.getTrainers(branchId);
    console.log(`👥 Trainers fetched - Count: ${trainers.length}`);
    responseHandler.success(res, 'Trainers fetched successfully', { trainers });
  } catch (error) {
    console.error('❌ Trainers Controller Error:', error);
    next(error);
  }
};
